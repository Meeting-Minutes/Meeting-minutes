...
- **Founding an org is open to anyone** (an org is like a server), and the founder is granted an org-wide **Admin** role. "Admin" here is not a special-cased concept in the code — it's a normal role seeded with the `ADMIN_PERMISSION_KEYS` set (`manage_roles`, `manage_members`, `manage_org`, `manage_teams`, `manage_templates`, `manage_tags`, `manage_team_roles`, `view_audit_log`) and deliberately **no `superuser`**. Any role holding `manage_roles`/`manage_members` can reshuffle permissions, so the founder's role can be split into narrower roles later with no code or schema change.
